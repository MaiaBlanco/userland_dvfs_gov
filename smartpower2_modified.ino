...
// Log voltage, current, and power to serial connection.
#define LOG_TO_SERIAL true
// Debug output for internet connection
#define WIFI_DEBUG_OUTPUT false
// Update interval for LCD in ms
...
void setup() {
    ...
    // Enable or disable diagnostics from wifi libraries
    USE_SERIAL.setDebugOutput(WIFI_DEBUG_OUTPUT);
    ...
    timerId = timer.setInterval(UPDATE_INTERVAL, handler);
...
void handler(void)
{
	if (onoff == ON) {
		...
		if (connectedLCD || connectedWeb || logClient || LOG_TO_SERIAL) {
			readPower();
		}
	}
